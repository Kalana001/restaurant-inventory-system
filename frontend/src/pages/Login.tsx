...
export const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to authenticate user. Check your credentials.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div>
              ...
              <div className="mt-1.5 relative">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm transition-all"
                />
              </div>
            </div>
            ...
            <div>
              <button
                type="submit"
                disabled={loading}
                className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-md text-sm font-bold text-white bg-primary hover:bg-opacity-95 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary active:scale-98 transition-all disabled:opacity-50"
              >
                {loading ? (
